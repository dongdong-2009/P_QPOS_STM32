...
#define PHPAL_I14443P4_SW_IS_S_BLOCK(bPcb)                                  \
	...
	/* TA415_X JLL EMVCo Pcd 30/11/2011 */
                                     ...
                                     )
{
...
#ifdef NXPBUILD__PH_EMV
    /* TA001 JLL EMVCo Pcd 24/10/2011 Req. 10.2 FWT + dFWT = 4480 clock cycles */
    ...
    *pRxLength = 0;
	...
	//HAL_USB_BulkIn(2, pTxBuffer, wInfLength);
	//HAL_USB_BulkIn(2, *ppRxBuffer, *pRxLength);
    ...
    do
    {
        ...
        if (pDataParams->bStateNow & PHPAL_I14443P4_SW_STATE_CHAINING_BIT)
        {
            ...
            if (!bBufferOverflow)
            {
                ...
                /* TA202_X JLL EMV Pcd 26/09/2011 */
                                        ...
                                        )
{
...
#ifdef NXPBUILD__PH_EMV
	/* TA402 JLL 03/02/2012 Req. 10.3.5.5 - Reset the Nack count*/
    uint8_t		PH_MEMLOC_REM bNackRetryCount = 0;
#endif
    ...
    do
    {
        ...
        if (!bInvalidBlock)
        {
...
#ifdef NXPBUILD__PH_EMV
                    /* TA204 JLL EMVCo Pcd 23/09/11 - Req 10.4 */
                    if (bWtxm == 0)
                    {
                        status = PH_ADD_COMPCODE(PH_ERR_PROTOCOL_ERROR, PH_COMP_PAL_ISO14443P4);
                        break;
            ...
            {
#ifdef NXPBUILD__PH_EMV
            	/* TA401_15 JLL EMVCo Pcd 29/09/2011 Req.10.3.5.9 */
            	status = PH_ADD_COMPCODE(PHPAL_I14443P4_ERR_RECOVERY_FAILED, PH_COMP_PAL_ISO14443P4);
            	break;
#endif
            ...
            {
#ifdef NXPBUILD__PH_EMV
            	/* TA421 JLL 05/10/2011 EMV Pcd  */
            	if (!PHPAL_I14443P4_SW_IS_WTX(pResp[PHPAL_I14443P4_SW_PCB_POS]))
            	{
            		/* Reset the count for iblock repetition */
            		bRetryCount = 0;
            		/* Reset the count for Nack count */
            		bNackRetryCount = 0;
            	}
#else
            	bRetryCount = 0;
        ...
        else
        {
            ...
            {
#ifdef NXPBUILD__PH_EMV
            	/* TA401_15 JLL EMVCo Pcd 29/09/2011 - Req. 10.3 */
            	status = PH_ADD_COMPCODE(PHPAL_I14443P4_ERR_RECOVERY_FAILED, PH_COMP_PAL_ISO14443P4);
            	break;
#endif

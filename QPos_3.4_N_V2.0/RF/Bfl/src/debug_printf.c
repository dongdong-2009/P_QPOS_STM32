...
uint8_t hex_to_ascii(uint8_t hexnum)
{
	if (hexnum > 9)
		return ('A' + hexnum - 10);
	else
 ...
 * @return		None
 **********************************************************************/
void _debug_puts_hex(uint8_t hexnum)
{
	uint8_t hex_to_display[5];

	hex_to_display[0] = '0';
	hex_to_display[1] = 'x';
	hex_to_display[2] = hex_to_ascii((hexnum >> 4) & 0x0F);
	hex_to_display[3] = hex_to_ascii(hexnum & 0x0F);
	hex_to_display[4] = '\0';
	_debug_puts(&hex_to_display);
}
 ...
 * @return		None
 **********************************************************************/
void _debug_puts_hex16(uint16_t hexnum)
{
	uint8_t hex_to_display[6];

	hex_to_display[0] = '0';
	hex_to_display[1] = 'x';
	hex_to_display[2] = hex_to_ascii((hexnum >> 12) & 0x0F);
	hex_to_display[3] = hex_to_ascii((hexnum >> 8) & 0x0F);
	hex_to_display[4] = hex_to_ascii((hexnum >> 4) & 0x0F);
	hex_to_display[5] = hex_to_ascii(hexnum & 0x0F);
	hex_to_display[6] = '\0';
	_debug_puts(&hex_to_display);
}

/*********************************************************************//**
 * @brief		Puts [num] hex number to UART port
 * @param[in]	p_hexnum	pointer to hex array
 * @param[in]	num			number (8-bit long)
 *
 * @return		None
 **********************************************************************/
void _debug_printf_hex(uint8_t *p_hexnum, uint8_t num)
{
	uint8_t i;

	for (i = 0; i < num; i ++)
	{
		_debug_puts_hex(p_hexnum[i]);
	}
}
#endif
#endif

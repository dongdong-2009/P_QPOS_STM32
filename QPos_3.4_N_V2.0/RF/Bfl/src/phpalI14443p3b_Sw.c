                                    ...
                                    ) 
{
...
#ifdef NXPBUILD__PH_EMV
    /* TB001 JLL EMVCo Pcd 24/10/2011 - Wait FDT PCD - Req 4.27 */
...
#ifdef NXPBUILD__PH_EMV
    /* TB107_X JLL EMVCo Pcd 18/10/2011 */
...
#ifdef NXPBUILD__PH_EMV
    /* TB201_X JLL 12/10/11 EmvCo Pcd  */
    /* The computation of FWT for FWI = 0, 1 gives a timing shorter than expected */
    if (pDataParams->bFwi <= 0x01)
    {
    	dwFwt += dwFwt >> 2;
    }
#endif
    ...
    /* TB306_12 JLL 29/11/11 */
    if ((PH_ERR_NOISE_ERROR == (statusTmp & PH_ERR_MASK)) && (wRespLength == 0))
    {
    	return PH_ADD_COMPCODE(PH_ERR_SPECIFIC_PROTOCOL_ERROR, PH_COMP_PAL_ISO14443P3B);
    }

    /* return error now if any */
    PH_CHECK_SUCCESS(statusTmp);

#ifdef NXPBUILD__PH_EMV_DEBUG_PRINT
    _DBG_PRINT_BUFFER(aCmd, 9);
    _DBG("success "); _DBG_PRINT_BUFFER(pResp, wRespLength);
#endif

    /* TB306_10 JLL 29/11/11 - Cid different from 0 */
    if (0x00 != (pResp[0] & 0x0F))
	{
    	return PH_ADD_COMPCODE(PH_ERR_SPECIFIC_PROTOCOL_ERROR, PH_COMP_PAL_ISO14443P3B);
	}
...
#ifdef NXPBUILD__PH_EMV
    /* TB305_X JLL 30/11/2011 EMVCo Pcd */
    uint32_t    PH_MEMLOC_REM dwFwt = 0;
#endif
...
#ifdef NXPBUILD__PH_EMV
    /* TB311_X JLL 12/10/2011 EMVCo Pcd - Req. 9.6.1.3 */
    if (PH_ERR_IO_TIMEOUT == (status & PH_ERR_MASK))
    {
        /* Wait FWT + dFWT with FWI = 4 (~ 5ms) */
        statusTmp = phhalHw_Wait(
            pDataParams->pHalDataParams,
            PHHAL_HW_TIME_MILLISECONDS,
            (uint16_t) ((PHPAL_I14443P3B_SW_FWT_MIN_US * (1 << PHPAL_I14443P3B_SW_FWI_DEFAULT)) / 1000));
...
#ifdef NXPBUILD__PH_EMV
    /* TA306_10 JLL 29/11/11 - Cid different from 0 */
    if (PH_ERR_SPECIFIC_PROTOCOL_ERROR == (status & PH_ERR_MASK))
	{
    	return status;
	}
#endif
...
#ifdef NXPBUILD__PH_EMV
    /* TB305_X JLL 12/10/2011 EMVCo Pcd - Req. 9.6.1.3 */
    if (PH_ERR_SUCCESS != (status & PH_ERR_MASK))
    {
        dwFwt = (uint32_t)(PHPAL_I14443P3B_SW_FWT_MIN_US * (1 << pDataParams->bFwi));
...
#ifndef NXPBUILD__PH_EMV
    /* TB_OPTI_1 Set RxDeafTime to 8 Bits - JLL 26/09/2011 Redundant code */

...
void SCSI_ReadToc_Cmd(unsigned char lun)
{
	...
	Transfer_Data_Request((unsigned char *)ReadTOC_Data, length);	
	Set_CSW (CSW_CMD_PASSED, SEND_CSW_ENABLE);//###
}
#endif

unsigned char g_PackageID;
unsigned int    g_UMSLen;
unsigned short g_UMSOffset;
unsigned short g_Sendlen;

void SCSI_Vender_Cmd(uint8_t lun)
{
	...
	if (Bot_State == BOT_IDLE)
	{
		...
		else
		{
			...
			if (TransferState == TXFR_IDLE )
			  {
				Bulk_Data_Buff[0] = (G_APDU.RLE +2);
				Bulk_Data_Buff[1] = (G_APDU.RLE +2)>>8;


				if(G_APDU.RLE)
					memcpy(Bulk_Data_Buff+2, G_APDU.pOutData, G_APDU.RLE);

				Bulk_Data_Buff[G_APDU.RLE+2] = G_APDU.SW1;
				Bulk_Data_Buff[G_APDU.RLE+3] = G_APDU.SW2;

				g_Sendlen = G_APDU.RLE+4;
				g_UMSOffset =0;
			...
			if (TransferState == TXFR_ONGOING )
			  {
			  	 usSendlen = (g_Sendlen > BULK_MAX_PACKET_SIZE)?BULK_MAX_PACKET_SIZE:g_Sendlen;
				 USB_SIL_Write(EP1_IN, Bulk_Data_Buff , usSendlen);

			    	SetEPTxCount(ENDP1, usSendlen);
			    	SetEPTxStatus(ENDP1, EP_TX_VALID);  
			    	g_UMSOffset += usSendlen;
			       g_Sendlen -= usSendlen;

			       CSW.dDataResidue -= usSendlen;	    
			  }
	...
	else if (Bot_State == BOT_DATA_OUT)
	{
		if(g_PackageID == 0)
		{
			G_APDU.CLA = Bulk_Data_Buff[0];
			G_APDU.INS = Bulk_Data_Buff[1];
			G_APDU.PP1 = Bulk_Data_Buff[2];
			G_APDU.PP2 = Bulk_Data_Buff[3];
			G_APDU.PP3 = Bulk_Data_Buff[4];
			G_APDU.PP4 = Bulk_Data_Buff[5];
			G_APDU.PP5 = Bulk_Data_Buff[6];
			...
			case 0xEF:
				...
				break;
			default:
				usInDatalen = G_APDU.PP4*256 + G_APDU.PP5;
				break;
			}
			...
			if(usInDatalen>(64-7))
			{
				g_UMSLen = usInDatalen+7 -64;
				memcpy(G_APDU.pInData, Bulk_Data_Buff+7, 64-7);
				g_UMSOffset = 64-7;
				g_PackageID++;
				SetEPRxStatus(ENDP2, EP_RX_VALID);
			...
			{
				g_UMSLen = 0;
				memcpy(G_APDU.pInData, Bulk_Data_Buff+7, usInDatalen);
		...
		else
		{
			...
			if(g_UMSLen>64)
			{
				memcpy(G_APDU.pInData+g_UMSOffset, Bulk_Data_Buff, 64);
				g_UMSOffset += 64;
				g_UMSLen -= 64;
				SetEPRxStatus(ENDP2, EP_RX_VALID);
			}
			...
			{
				memcpy(G_APDU.pInData+g_UMSOffset, Bulk_Data_Buff, g_UMSLen);
	...
	else if (Bot_State == BOT_DATA_IN)
	{
			  ...
			  {
			  	 usSendlen = (g_Sendlen > BULK_MAX_PACKET_SIZE)?BULK_MAX_PACKET_SIZE:g_Sendlen;
				 USB_SIL_Write(EP1_IN, Bulk_Data_Buff+g_UMSOffset , usSendlen);
